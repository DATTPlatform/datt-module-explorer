<template>
  <div class="bottom bg-gray">
    <ul class="b_ul w1200">
      <li class="b_li font14 fl capitalize" v-show="symbol ==='DATT'">
        <a href="https://datt.one" target="_blank">{{$t('bottom.website')}}</a>
      </li>
      <li class="b_li font14 fl" v-show="symbol ==='DATT'">
        <a href="https://github.com/DATTPlatform/DATT" target="_blank">Github</a>
      </li>
      <li class="b_li font14 fl capitalize" v-show="symbol ==='DATT'">
        <a href="https://wallet.datt.one/" arget="_blank">{{$t('bottom.webWallet')}}</a>
      </li>
      <li class="b_li font14 fl capitalize" v-show="symbol ==='DATT'">
        <a href="https://datt.one/" target="_blank">{{$t('bottom.community')}}</a>
      </li>
      <li class="b_li font14 fl capitalize click" @click="toUrl('protocolUpdate')" v-show="symbol ==='DATT'">
  ...
  export default {
    ...
    created() {
      ...
      setInterval(() => {
        ...
        this.getNodeNumber();
        this.getNULSNumber();
        this.symbol = sessionStorage.hasOwnProperty('symbol') ? sessionStorage.getItem('symbol') : 'DATT';
        document.title = this.symbol + " Explorer";
      }, 10000);
    },
    mounted() {
